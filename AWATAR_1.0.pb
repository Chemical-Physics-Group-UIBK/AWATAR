; AWATAR vs. 1.0
; (c) 2006, 2008, 2009, 2021, 2022, 2023, 2025 Martin K. Beyer, TU Berlin, Christian-Albrechts-Universität zu Kiel, Universität Innsbruck
; (c) 2022  Milan Oncák, Universität Innsbruck
; (c) 2022, 2023, 2024, 2025, 2026 Magdalena Salzburger, Universität Innsbruck
...
; new in AWATAR vs. 1.0j on 10.11.25: Changes affecting MEM with 2 groups and isomerization between groups: thermalization is only applied to Group 0. But maybe we should have a parameter to choose between "Group 0 only" or "all" for the thermalization. 
;                            Some bugs fixed in section "-Restore Equilibrium between Wells" of MEM, bug only affected group isomerization - which has not been used so far for production purposes - MKB
;                        on 12.11.25: several bugs fixed in section "AddIR Modes of 2nd TS fragment (for loose TS)", now also a single number can be given - MKB
; new in AWATAR vs. 1.0k on 10.12.25: bug found in ">Print RRKM Rates k(E)", gosub print_rates: in totaldos calculations, wells were not shifted with their relative energy. Bug is fixed. Error only in this subroutine, in MEM routine calculation of RRKM rate constants was correct
; new in AWATAR vs. 1.0l on 15.01.25: new parameter (Print lines where results are zero (YES/NO)) for the results of RRKM-rates and SoS. Isomerisation channels uncommented.
; new in AWATAR vs. 1.0m on 23.01.25: avoid crashing if non existing wells/TS are defined in channel definition.
...
IncludeFile "AWATAR_1.0_include.pb"
...
printzerolines_flag.l = 0                   ; If parameter set to YES, lines where results are zero are printed for calculation of RRKM rates and SoS.
	...
	Repeat
		...
		Select EventGadget()
		  ...
		  Gosub remove_multiple_panels
			Case #Button_15  ; new in AWATAR 1.0 on 20.02.26
      Gosub about
		EndSelect
...
add_ts:
  ...
  to_add$ = Trim(InputRequester("Add 2nd TS fragment",
							    "To which TS do you want to add the 2nd TS fragment? Input is in the form m or m-n. E.g.: 5, 0-3", "")) ; changed to allow also individual numbers for single TS - MKB
  Define dashPos.i, startVal.i, endVal.i, tserrorflag.i = 0 ; tserrorflag introduced to skip reading if input is not correct
  dashPos = FindString(to_add$, "-", 1)
  If dashPos <= 0 ; no range given if yes
    startVal = Val(to_add$) 
    endVal = startVal
    If startVal = 0 And FindString(to_add$, "0", 1 ) = 0 ; make sure that a value is given, even if zero
      MessageRequester("Error", "Input must be in the form m or m-n.", 0)
      tserrorflag = 1
    EndIf
  Else  ; range given 
    ...
    Else
      If endVal < startVal
        MessageRequester("Error", "Input must be in the form m or m-n in ascending order.", 0)
        tserrorflag = 1
      EndIf
    EndIf
  EndIf
  
 If tserrorflag = 0 ; skip reading if syntax error found
    ...
    If Left(GetGadgetItemText(#Panel_0,k),4)="Well"
      ...
      While j<CountGadgetItems(#Editor_0+k)
        ...
        If FindString(text$,"pib1d mass a",1) ; 1D particle in a box
          ...
          EndIf
          Gosub parse
          aqsm_mass.d = ValD(parse$)*amu ; mass of particle in in amu converted to kg
          Gosub parse
          pib_a.d = ValD(parse$)*1e-10 ; box length in Angstrom converted to meter
        ...
        If FindString(text$,"pib2d mass a b",1) ; 2D particle in a box
          ...
          pib_a.d = ValD(parse$)*1e-10 ; box length in Angstrom converted to meter
          Gosub parse
          pib_b.d = ValD(parse$)*1e-10 ; box length in Angstrom converted to meter
        ...
        If FindString(text$,"pib3d mass a b c",1) ; 3D particle in a box
          ...
          EndIf
          Gosub parse
          aqsm_mass.d = ValD(parse$)*amu ; mass of particle in amu converted to kg
          Gosub parse
          pib_a.d = ValD(parse$)*1e-10 ; box length in Angstrom converted to meter
          Gosub parse
          pib_b.d = ValD(parse$)*1e-10 ; box length in Angstrom converted to meter
          Gosub parse
          pib_c.d = ValD(parse$)*1e-10 ; box length in Angstrom converted to meter
          pib_gsa.d = h_planck*h_planckSI/(8*aqsm_mass*pib_a*pib_a) ; ground state energy of 1D PIB dimension a
          pib_gsb.d = h_planck*h_planckSI/(8*aqsm_mass*pib_b*pib_b) ; ground state energy of 1D PIB dimension b
          pib_gsc.d = h_planck*h_planckSI/(8*aqsm_mass*pib_c*pib_c) ; ground state energy of 1D PIB dimension c
          zpe_aqsm_well(i_wells) = zpe_aqsm_well(i_wells)+pib_gsa+pib_gsb+pib_gsc; add ground state energy to zero-point correction
          n_piba.l = 0
          Repeat
            n_piba = n_piba+1
            n_pibb.l = 0
...
print_dos:
  ...
  Else
    If DoNotClearResults_flag = 0
      ...
      AddGadgetItem(#Editor_0,-1,"AWATAR 1.0 © 2026 M. Beyer, M. Salzburger, M. Ončák")
...
print_rates:
Gosub read_channels
 If ChannelNotValid_Flag = 0  ;Continue only if wells and TS defined in the channels exist
 ...
 ;     EndIf
  ...
  EndIf
  AddGadgetItem(#Editor_0,-1,"")
  AddGadgetItem(#Editor_0,-1,"RRKM Rate Constants in s-1, Total Energy in cm-1:")
  line$ = ""
  For i_channels = 0 To n_channels-1
  ...
  EndIf
  AddGadgetItem(#Editor_0,-1,"Total_Energy   "+line$)
;  line$ = ""  ; remove units, not practical for copying in Excel
;  For i_channels = 0 To n_channels-1
...
;  AddGadgetItem(#Editor_0,-1,"(cm-1)         "+line$)
  ...
  Repeat
    ...
    EndIf  
    For i_channels = 0 To n_channels-1
      If well_channel(i_channels) = -1 ;  If channel uses Well all
        If ts_channel(i_channels) >= 0  ; obsolete option, cannot occur starting vs. 3.52
         If Int((total_en-ts_relative(ts_channel(i_channels)))/bs_quantum+0.5) > 0
            totaldos.d = 0
            For i_wells = 0 To n_wells-1
              If total_en-well_relative(i_wells) > 0
                totaldos = totaldos+dos_well(i_wells,Int((total_en-well_relative(i_wells))/bs_quantum+0.5))
              EndIf
            Next i_wells
          ...
          EndIf
        Else
          If ts_channel(i_channels) = -1   ; If channel uses TS all
            totaldos.d = 0
            For i_wells = 0 To n_wells-1
              If total_en-well_relative(i_wells) > 0
                totaldos = totaldos+dos_well(i_wells,Int((total_en-well_relative(i_wells))/bs_quantum+0.5))
      ...
      Else
        ...
        EndIf
      EndIf
      line$=line$+LSet(science(rate,4),15) ;   +LSet(science(totalsos,4),15)+LSet(science(totaldos,4),15) ; used for Debugging in vs. 1.0k
    Next i_channels
...
read_channels:
  ChannelNotValid_Flag = 0 ; This flag is used to interupt procedure if non existing wells or TS are used in the channel definition.
  n_channels = 0
  ...
  While j<CountGadgetItems(#Editor_1)
    If FindString(text$,"channel",1)
      ...
      If parse$="all"
        ...
        Else
          ...
          Repeat
            ...
            If ts_parallel(i_channels,n_ts_parallel(i_channels)) >= n_ts
              Ergebnis = MessageRequester("Error","Channel definition not valid. Used ts does not exist",#PB_MessageRequester_Ok )
              ChannelNotValid_Flag = 1
            EndIf
      ...
      Else
        ...
        If well_channel(i_channels) >= n_wells
          Ergebnis = MessageRequester("Error","Channel definition not valid. Used well does not exist",#PB_MessageRequester_Ok )
          ChannelNotValid_Flag = 1
        EndIf  
        Gosub parse ; read "ts"
        Gosub parse ; read #TS
        ts_channel(i_channels) = Val(parse$)
        If ts_channel(i_channels) >= n_ts
          Ergebnis = MessageRequester("Error","Channel definition not valid. Used ts does not exist",#PB_MessageRequester_Ok )
          ChannelNotValid_Flag = 1
        EndIf
      EndIf
...
j = 0
  ...
  While j<CountGadgetItems(#Editor_1)
    ...
    If FindString(text$,"print lines where results are zero (yes/no):",1)  ;new in vs. AWATAR_1.0l
      Repeat
        Gosub parse
      Until parse$ = "(yes/no):"
      Gosub parse
      If parse$ = "yes"
        printzerolines_flag.l = 1
      EndIf
    EndIf     
    ...
    If FindString(text$,"rate of photon absorption for well",1)
      ...
      Else ; If "Rate of Photon absorption for Well 0 3 7 in s-1:" is used.
        ...
        Until parse$ = "s-1:"
        Gosub parse
        k_ir.d = ValD(parse$) ;This is the rate of photon absorption - bugfix: was Val(parse$), would not read scientific notiation, e.g. 1e5 - MKB
...
initialize_population:  ; subroutine to initialize a population at temperature t_amb before master equation modeling or copy existing population into active array
  ...
  If flag_population.l = 0   ; no population available, initialize with Boltzmann
    If t_amb = 0
      For i_wells = 0 To n_wells-1  
        If group(i_wells) = 0  ; new in vs. AWATAR 1.0j thermalize only group 0 isomers
...
MEM_iteration:    ; iteration of MEM without normalization; one time step d_t is handeled
      ...
      ;new procedure for calculation equilibium since vs.3.44:  If isomerisation rates trough TS would be very high, the isomerisation takes place without barrier (chemical equilibrium). Only for bins and timesteps, where the rates are low enough, Isomerisation takes place via the TS.
      ...
      lowest_TS_Energy.d = 1e10   ; bug fix (?) in vs. AWATAR_1.0j, was = 0 before (MKB)      
      fee_barrier.d = 0
      fee_FreeExchange.d = 0
      If n_isoreact > 0
         ...
         For i_fee = 0 To n_fee
           ...
           EndIf
         Next i_fee
         fee_FreeExchange = n_fee ; added in vs. AWATAR_1.0j, prevent fee_FreeExchange = 0 (MKB)
         For i_fee = fee_barrier To n_fee
           If total_iso_depletion_SumAllWells(i_fee) > FreeExchange_Criterion   ; bug fix (?) in vs. AWATAR_1.0j, was "<" before (MKB)
      ...
      For j = 0 To n_fee  
        ...
        ElseIf n_isochan = 0 Or j =< fee_barrier   ;In this case, all groups are separated, chemical equilibrium only within the group.
